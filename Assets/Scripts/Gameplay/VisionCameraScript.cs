...
public class VisionCameraScript : MonoBehaviour {
	...
	void FixedUpdate(){
		...
		hit = Physics2D.Raycast (transform.position, transform.up, 4.0f , fov_hit);

		if (hit!=null && hit.transform != null) {
			
			Vector3 distance = new Vector3(hit.point.x, hit.point.y, 0) - transform.position;
			distance.z = 0;
			transform.localScale = new Vector3 (Mathf.Clamp(Vector3.Magnitude(distance) / 4.0f, 0.05f, 1),  Mathf.Clamp(distance.magnitude / 4.0f, 0.05f, 1) , transform.localScale.z);
